...
class ProgressStore {
  ...
  /// The store if [init] has already run, else null.
  ///
  /// For callers that are *opportunistic* rather than essential — a background
  /// prefetch caching its result, say. Throwing at them would turn "the cache
  /// was not available" into a crash, and the whole point of that code is that
  /// it can be skipped.
  static ProgressStore? get instanceOrNull => _instance;
  ...
  // Separate from the save slot above, which stores *progress* on a board (which
  // arrows have left). This stores the board itself, so a level that is slow to
  // generate can be opened without generating it again.

  static const _prefetchVersion = 1;

  /// How many boards are kept: the one being played and the one after it.
  ///
  /// Two, because those are the only levels anyone opens next. A deeper cache
  /// would spend storage and prefs-write time on levels the player has either
  /// already finished or will not reach this session.
  static const _prefetchSlots = 2;

  String _prefetchKey(String gameId) => 'prefetch_$gameId';

  /// Files [board] (a `SnakeBoard.toJson()`-shaped payload) as the ready-made
  /// board for [level], stamped with the [generation] that built it.
  ///
  /// [generation] is the guard that matters. A board generated by an older build
  /// is not merely stale, it is *wrong*: the level would open with a board today's
  /// generator would never produce for that number. Stamping the payload means a
  /// generator change drops the cache instead of resurrecting it.
  void savePrefetchedBoard(
      String gameId, int level, int generation, Map<String, dynamic> board) {
    final kept = <Map<String, dynamic>>[
      {'level': level, 'board': board},
    ];
    for (final entry in _prefetchEntries(gameId, generation)) {
      if (kept.length >= _prefetchSlots) break;
      if (entry['level'] == level) continue; // just replaced
      kept.add(entry);
    }
    _prefs.setString(
        _prefetchKey(gameId),
  ...
  /// The stored board for [level], or null if there isn't a usable one from this
  /// [generation]. Never throws: a missing cache entry must only ever cost time.
  Map<String, dynamic>? loadPrefetchedBoard(
      String gameId, int level, int generation) {
    for (final entry in _prefetchEntries(gameId, generation)) {
  ...
  /// Whether a usable stored board exists, without rebuilding it.
  bool hasPrefetchedBoard(String gameId, int level, int generation) =>
      loadPrefetchedBoard(gameId, level, generation) != null;

  void clearPrefetchedBoards(String gameId) =>
      _prefs.remove(_prefetchKey(gameId));

  List<Map<String, dynamic>> _prefetchEntries(String gameId, int generation) {
    final raw = _prefs.getString(_prefetchKey(gameId));
    if (raw == null) return const [];
    try {
      final decoded = jsonDecode(raw);
      if (decoded is! Map<String, dynamic>) return const [];
      if (decoded['v'] != _prefetchVersion) return const [];
      if (decoded['gen'] != generation) return const [];
      final boards = decoded['boards'];
